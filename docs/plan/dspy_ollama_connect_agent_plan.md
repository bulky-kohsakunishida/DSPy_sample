...
DSPy と Ollama の `gemma4:12b` を使い、Amazon Connect AI Agents のようなマルチターン会話エージェントを想定したプロンプトチューニングの動作検証を行う。
...
6. 評価データセットのレビューと固定化
7. DSPy optimizer によるプロンプトチューニング
8. 改善前後の比較レポート作成
...
最初の最適化対象は `NextActionPlanner` に限定する。Amazon Connect AI Agents のプロンプトチューニングに近い価値が出やすく、他モジュールより評価しやすいため。
...
- 最終採用プロンプトは `outputs/prompts/current.json` から参照できるようにする
